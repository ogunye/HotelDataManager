...
using HostelDataManagerDomain.Exceptions;
...
namespace HostelDataManagerServices
{
    internal sealed class AuthenticationService : IAuthenticationService
    {
        ...
        private SigningCredentials GetSigningCredentials()
        {
            ...
            var secert = new byte[64];
            Array.Copy(key, secert, Math.Min(key.Length, secert.Length));
            var securityKey = new SymmetricSecurityKey(secert);
        ...
        private ClaimsPrincipal GetPrincipalFromExipredToken(string token)
        {
            var jwtSettings = _configuration.GetSection("jwtSettings");
            var tokenValidationParameters = new TokenValidationParameters
            {
                ...
                ValidateIssuerSigningKey = true, IssuerSigningKey = new SymmetricSecurityKey(
                    Encoding.UTF8.GetBytes(Environment.GetEnvironmentVariable("SECRET") ?? "mydefaultkey")),
        ...
        private ClaimsPrincipal GetPrincipalFromExpiredToken(string token)
        {
            ...
            var tokenValidationParameters = new TokenValidationParameters
            {
                ...
                IssuerSigningKey = new SymmetricSecurityKey(
                    Encoding.UTF8.GetBytes(Environment.GetEnvironmentVariable("SECERT") ?? "mydefaultkey")),
                ValidateLifetime = true,
                ValidIssuer = jwtSettings["validIssuer"],
                ValidAudience = jwtSettings["validAudience"]
            };
